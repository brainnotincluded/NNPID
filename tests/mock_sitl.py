...
class MockState:
    ...
    # Setpoints
    position_setpoint: np.ndarray | None = None
    velocity_setpoint: np.ndarray | None = None
...
class MockSITL:
    ...
    def _handle_sensor(self, msg) -> None:
        ...
        with self._lock:
            ...
            np.array([msg.xacc, msg.yacc, msg.zacc])
    ...
    def get_state(self) -> MockState:
        ...
        with self._lock:
            return MockState(
                ...
                position_setpoint=self.state.position_setpoint.copy()
                if self.state.position_setpoint is not None
                else None,
                velocity_setpoint=self.state.velocity_setpoint.copy()
                if self.state.velocity_setpoint is not None
                else None,
