...
def monitor_training(run_dir: str, interval: int = 30):
    ...
    print(f"Target: 20,000,000 timesteps")
    ...
    while True:
        try:
            ...
            if last_timesteps > 0:
                ...
                eta_seconds = (20_000_000 - current_steps) / steps_per_sec if steps_per_sec > 0 else 0
            else:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        ...
        run_dirs = sorted(runs_dir.glob("yaw_tracking_*"), key=lambda x: x.stat().st_mtime, reverse=True)
        if not run_dirs:
