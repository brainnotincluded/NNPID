...
def load_model(model_path: Path) -> Optional[BaseAlgorithm]:
    """Load trained model.
...
def plot_metrics(
    ...
    # Prepare data
    patterns = list(set(m.target_pattern for m in all_metrics))
    ...
    for pattern in patterns:
        ...
        ax.bar(patterns.index(pattern), np.mean(errors), 
    ...
    for pattern in patterns:
        ...
        ax.bar(patterns.index(pattern), np.mean(tracking),
    ...
    ax.pie([not_crashed, crashed], labels=['Success', 'Crashed'],
...
def evaluate(
    ...
    for pattern in test_patterns:
        ...
        for ep in range(episodes_per_pattern):
            ...
            print(f"    Episode {ep+1}: reward={metrics.total_reward:.1f}, "
    ...
    # Save plot
    if save_plot_path or True:  # Always show plot
