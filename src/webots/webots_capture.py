...
def main():
    ...
    try:
        ...
        print(
            f"Cannot connect to {WEBOTS_CAMERA}:{WEBOTS_CAMERA_PORT}: {e}",
            file=sys.stderr,
            flush=True,
        )
        print(
            "Ensure Webots is running with the camera stream server on that host/port.",
            file=sys.stderr,
            flush=True,
        )
        sys.exit(1)
    print(
        f"Connected to {WEBOTS_CAMERA}:{WEBOTS_CAMERA_PORT}. Receiving frames (press 'q' to quit).",
        flush=True,
    )
