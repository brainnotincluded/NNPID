...
class WindPerturbation(BasePerturbation):
    ...
    def _update_dryden_turbulence(self, dt: float, state: QuadrotorState) -> None:
        ...
        scale_uv = 200 * (altitude / 20) ** 0.5  # meters
        scale_w = 50  # meters

        # Turbulence intensities
        sigma_w = 0.1 * cfg.turbulence_intensity * cfg.steady_wind_velocity
        sigma_uv = sigma_w / (0.177 + 0.000823 * altitude) ** 0.4

        # Airspeed (use magnitude of velocity)
        airspeed = max(0.1, np.linalg.norm(state.velocity))

        # White noise input
        noise = self._rng.normal(0, 1, 3)

        # First-order Markov process for each axis
        scales = [scale_uv, scale_uv, scale_w]
        sigmas = [sigma_uv, sigma_uv, sigma_w]
        for i, (scale, sigma) in enumerate(zip(scales, sigmas, strict=True)):
            tau = scale / airspeed
            alpha = dt / tau
            self._turbulence_velocity[i] = (1 - alpha) * self._turbulence_velocity[
                i
    ...
    def _update_thermals(self, dt: float, state: QuadrotorState) -> None:
        ...
        if not self._thermal_active and self._rng.random() < cfg.thermal_probability * dt:
