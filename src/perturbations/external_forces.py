...
class VibrationModel:
    ...
    def get_force(
        ...
        if self.motor_coupled:
            coupling = np.mean(motor_speeds)
        else:
            coupling = 1.0
...
class EMIModel:
    ...
    def update(self, time: float, dt: float) -> None:
        ...
        if self._burst_active:
            ...
            self._burst_effect = self._rng.uniform(
                -self.max_magnitude, self.max_magnitude, 4
            )
...
class ExternalForcesPerturbation(BasePerturbation):
    ...
    def __init__(self, config: Optional[ExternalForcesConfig] = None):
    ...
    def _update_impulses(self, dt: float) -> None:
        ...
        self._impulse_events = [
            e for e in self._impulse_events
            if e.is_active(self._time) or e.start_time > self._time
        ]
        
        # Check for new impulse
        time_since_last = self._time - self._last_impulse_time
        if time_since_last > cfg.impulse_min_interval:
            if self._rng.random() < cfg.impulse_probability:
    ...
    def _generate_impulse(self) -> None:
        ...
        force_magnitude = self._rng.uniform(
            cfg.impulse_force_min, cfg.impulse_force_max
        )
        ...
        # Random torque
        torque_magnitude = self._rng.uniform(
            cfg.impulse_torque_min, cfg.impulse_torque_max
        )
    ...
    def get_info(self) -> Dict[str, Any]:
        ...
        info = super().get_info()
        info.update({
            "active_impulses": len([e for e in self._impulse_events if e.is_active(self._time)]),
            "total_impulses_generated": len(self._impulse_events),
            "emi_active": self._emi.is_active,
            "vibration_force": self._vibration.get_force(self._time, self._motor_speeds).tolist(),
        })
        return info
