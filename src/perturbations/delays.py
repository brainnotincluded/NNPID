...
class DelayBuffer:
    ...
    def get_delayed(self, current_time: float, delay_ms: float) -> np.ndarray | None:
        """Get delayed sample.
...
class FirstOrderFilter:
    ...
    def reset(self, initial_value: np.ndarray | None = None) -> None:
        """Reset filter state.
...
class DelayPerturbation(BasePerturbation):
    ...
    def __init__(self, config: DelayConfig | None = None):
        ...
        self._last_valid_observation: np.ndarray | None = None
        self._last_valid_action: np.ndarray | None = None
    ...
    def _update_jitter(self) -> None:
        ...
        if cfg.jitter_distribution == "uniform":
            self._current_jitter = self._rng.uniform(cfg.jitter_base, cfg.jitter_max, 4)
            ...
            self._current_jitter = np.clip(
                self._rng.normal(mean, std, 4), cfg.jitter_base, cfg.jitter_max
            )
        elif cfg.jitter_distribution == "exponential":
            scale = (cfg.jitter_max - cfg.jitter_base) / 2
            self._current_jitter = np.clip(
                cfg.jitter_base + self._rng.exponential(scale, 4), cfg.jitter_base, cfg.jitter_max
            )
        else:
            self._current_jitter = np.zeros(4)
    ...
    def _check_dropout(self) -> bool:
        ...
        # Check probability and consecutive limit
        should_drop = (
            self._rng.random() < cfg.dropout_probability
            and self._consecutive_drops < cfg.dropout_max_consecutive
        )
        if should_drop:
    ...
    def apply_to_observation(self, obs: np.ndarray) -> np.ndarray:
        ...
        delayed_obs = self._observation_buffer.get_delayed(self._time, self._effective_imu_delay)
