...
class BiasRandomWalk:
    ...
    def __init__(self, drift_rate: float, initial_bias: Optional[np.ndarray] = None):
        ...
        self._bias: Optional[np.ndarray] = initial_bias
...
class GPSOutage:
    ...
    def __init__(
        ...
        self._last_valid_position: Optional[np.ndarray] = None
        self._last_valid_velocity: Optional[np.ndarray] = None
        self._rng: Optional[np.random.Generator] = None
    ...
    def update(self, time: float, position: np.ndarray, velocity: np.ndarray) -> None:
        ...
        if self._is_lost:
            ...
            if self._rng.random() < self.loss_probability:
                ...
                self._outage_duration = self._rng.uniform(
...
class SensorNoisePerturbation(BasePerturbation):
    ...
    def __init__(self, config: Optional[SensorNoiseConfig] = None):
        ...
        self._outlier_sensor: Optional[SensorType] = None
    ...
    def _update_outliers(self, dt: float) -> None:
        ...
        if self._outlier_active:
            ...
            sensor_types = [
                SensorType.GYRO, SensorType.ACCEL, 
                SensorType.POSITION, SensorType.VELOCITY
            ]
            ...
            self._outlier_value = direction * base * cfg.outlier_magnitude * self._rng.uniform(0.5, 1.5, 3)
