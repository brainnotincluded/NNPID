...
class GroundEffect:
    ...
    def get_thrust_multiplier(self, altitude: float) -> float:
        ...
        height_ratio = altitude / self.rotor_radius

        if height_ratio < 0.1:
            height_ratio = 0.1  # Avoid singularity
...
class PhysicsPerturbation(BasePerturbation):
    ...
    def reset(self, rng: np.random.Generator) -> None:
        ...
        if cfg.com_offset_enabled:
            if cfg.com_randomize:
                self._com_offset = rng.uniform(-cfg.com_max_offset, cfg.com_max_offset, 3)
            else:
        ...
        if cfg.mass_change_enabled:
            self._mass_multiplier = rng.uniform(cfg.mass_variation_min, cfg.mass_variation_max)
        else:
            self._mass_multiplier = 1.0
        ...
        self._motor_failure_checked = False
        should_check_failure = (
            cfg.degradation_enabled
            and cfg.degradation_failure_probability > 0
            and rng.random() < cfg.degradation_failure_probability
        )
        if should_check_failure:
            # Random motor failure at random time (handled in update)
    ...
    def _randomize_parameters(self) -> None:
        ...
        if cfg.com_randomize and cfg.com_offset_enabled:
            self._com_offset = self._rng.uniform(-cfg.com_max_offset, cfg.com_max_offset, 3)
    ...
    def update(self, dt: float, state: QuadrotorState) -> None:
        ...
        if cfg.degradation_enabled:
            for motor in self._motors:
                motor.degradation = min(1.0, motor.degradation + cfg.degradation_rate * dt)
