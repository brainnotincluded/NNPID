...
from .base import BasePerturbation, AerodynamicsConfig
...
class AirDensityModel:
    ...
    def reset(self, rng: np.random.Generator) -> None:
        ...
        self._rng = rng
        self._current_variation = rng.uniform(
            -self.random_variation, self.random_variation
        )
...
class BladeFlappingModel:
    ...
    def get_forces_and_moments(
        ...
        h_force_x = -self.flapping_coefficient * thrust * velocity[0] / (V_horizontal + 0.1)
        h_force_y = -self.flapping_coefficient * thrust * velocity[1] / (V_horizontal + 0.1)
...
class VortexRingState:
    ...
    def update(
        ...
        vrs_boundary = self.descent_threshold * (1.0 + 0.3 * horizontal_velocity / self.induced_velocity)
...
class AerodynamicsPerturbation(BasePerturbation):
    ...
    def __init__(self, config: Optional[AerodynamicsConfig] = None):
    ...
    def update(self, dt: float, state: "QuadrotorState") -> None:
        ...
        if cfg.blade_flapping_enabled:
            ...
            self._flapping_force, self._flapping_moment = self._blade_flapping.get_forces_and_moments(
    ...
    def _calculate_drag(self, state: "QuadrotorState") -> None:
        ...
        drag_magnitude = (
            0.5 * self._current_density * cfg.drag_coefficient * 
            cfg.reference_area * speed ** 2
        )
    ...
    def apply_to_action(self, action: np.ndarray) -> np.ndarray:
        ...
        if cfg.density_variation_enabled:
            density_ratio = self._current_density / cfg.air_density
