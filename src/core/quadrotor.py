...
class QuadrotorConfig:
    ...
    motor_directions: np.ndarray = field(default_factory=lambda: np.array([1, -1, 1, -1]))
...
class QuadrotorDynamics:
    ...
    def __init__(self, config: QuadrotorConfig | None = None):
    ...
    def wrench_to_motor_thrusts(self, wrench: np.ndarray) -> np.ndarray:
        ...
        # Clip to valid range
        thrusts = np.clip(
            thrusts, self.config.min_thrust_per_motor, self.config.max_thrust_per_motor
        )
        return thrusts
