...
class YawTrackerSITL:
    ...
    def __init__(
        ...
        config: Optional[YawTrackerSITLConfig] = None,
    ):
    ...
    def _handle_message(self, msg) -> None:
        ...
        if msg_type == "LOCAL_POSITION_NED":
            ...
            with self._state_lock:
                ...
                self._attitude = Rotations.euler_to_quaternion(
    ...
    def send_guided_velocity(
        ...
        try:
            ...
            type_mask = (
                0b0000_0001_1100_0111  # Control velocity and yaw rate
            )
    ...
    def step(self, dt: float) -> Tuple[float, float]:
        ...
        if self.model is not None:
            ...
            yaw_rate_cmd = np.clip(yaw_rate_cmd, -self.config.max_yaw_rate, self.config.max_yaw_rate)
    ...
    def run_loop(
        ...
        if errors:
            ...
            self._metrics["tracking_percentage"] = (
                100 * np.mean([1 if abs(e) < 0.1 else 0 for e in errors])
            )
...
def main():
    ...
    try:
        if tracker.connect():
            if tracker.arm_and_takeoff(altitude=args.altitude):
