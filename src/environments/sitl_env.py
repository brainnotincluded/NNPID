...
from ..communication.mavlink_bridge import MAVLinkConfig
from ..communication.messages import (
    HILGPSMessage,
    HILSensorMessage,
)
from ..controllers.offboard_controller import (
    OffboardConfig,
    OffboardController,
    OffboardControlMode,
)
...
class SITLEnv(gym.Env):
    ...
    def __init__(
        self,
        config: SITLEnvConfig | None = None,
    ...
    def _step_physics(self, action: np.ndarray) -> None:
        ...
        for _i in range(self._physics_steps_per_control):
    ...
    def _is_terminated(self) -> bool:
        ...
        # Too far
        return (
            np.linalg.norm(state.position - self._target_position) > self.config.max_position_error
        )
