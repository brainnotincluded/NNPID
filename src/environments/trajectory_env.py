...
from ..utils.trajectory import TrajectoryGenerator, TrajectoryType, TrajectoryPoint
...
class TrajectoryEnv(BaseDroneEnv):
    ...
    def __init__(
        self,
        config: Optional[TrajectoryEnvConfig] = None,
        ...
        self._current_reference: Optional[TrajectoryPoint] = None
    ...
    def _setup_trajectory(self) -> None:
        ...
        if cfg.randomize_trajectory:
            radius = self._np_random.uniform(
                cfg.radius_range[0], cfg.radius_range[1]
            )
            altitude = self._np_random.uniform(
                cfg.altitude_range[0], cfg.altitude_range[1]
            )
    ...
    def _get_info(self) -> Dict[str, Any]:
        ...
        if self._step_count > 0:
            info["average_tracking_error"] = (
                self._total_tracking_error / self._step_count
            )
