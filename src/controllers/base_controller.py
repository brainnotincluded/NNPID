...
class PIDController(BaseController):
    ...
    def compute_action(
        ...
        # Derivative
        if dt > 0:
            pos_error_dot = (pos_error - self._prev_error) / dt
        else:
            pos_error_dot = np.zeros(3)
        ...
        if accel_norm > 0.01:
            ...
            desired_roll = np.arcsin(
                np.clip(desired_accel[1] / accel_norm, -1, 1)
            )
            desired_pitch = np.arcsin(
                np.clip(-desired_accel[0] / accel_norm, -1, 1)
            )
        else:
            desired_roll = 0.0
            desired_pitch = 0.0
